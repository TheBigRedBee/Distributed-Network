...
class CRCServer(object):
    ...
    def setup_server_socket(self):
        ...
        self.sel.register(self.server_socket, self.events, None)
    ...
    def connect_to_server(self):
        ...
        try:
            connect_socket.connect((self.connect_to_host_addr, int(self.connect_to_port)))
        except:
        ...
        self.sel.register(connect_socket, events, my_sock_data)     
    ...
    def check_IO_devices_for_messages(self):
        ...
        while not self.request_terminate:
            # TODO: Implement the above functionality
            ready_IO = self.sel.select(timeout=2)
            for io_device, event_mask in ready_IO:
                my_sock_data = io_device.data

                if my_sock_data == None:
    ...
    def cleanup(self):
        # TODO: Implement the above functionality
        connected_sockets = list(self.sel._fd_to_key.values())
    ...
    def accept_new_connection(self, select_key):
        # TODO: Implement the above functionality
        sock, addr = select_key.fileobj.accept()
        sock.setblocking(False)
        events = selectors.EVENT_READ | selectors.EVENT_WRITE
        my_sock_data = ConnectionData()
        my_sock_data.sock = sock
        self.sel.register(sock, events, my_sock_data)
    ...
    def handle_io_device_events(self, io_device, event_mask):
        ...
        sock_data = io_device.data 

        if event_mask & selectors.EVENT_READ:
            msg = sock.recv(1024)
            if msg:
                self.handle_messages(io_device, msg) # io_device.data
            else:
                self.sel.unregister(sock)
                sock.close()
        if event_mask & selectors.EVENT_WRITE:
            if len(sock_data.write_buffer) > 0:
                try:
                    print("sending the data")
                    sock.send(sock_data.write_buffer)
                    sock_data.write_buffer = b''
                except:
    ...
    def broadcast_message_to_servers(self, message, ignore_host_id=None):
        # TODO: Implement the above functionality
        for server in self.adjacent_server_ids:
            if server != ignore_host_id:
                self.send_message_to_host(server, message)
    ...
    def broadcast_message_to_adjacent_clients(self, message, ignore_host_id=None):
        # TODO: Implement the above functionality
        for key in self.adjacent_user_ids:
            if key != ignore_host_id:
                self.send_message_to_host(key, message)
    ...
    def handle_server_registration_message(self, io_device, message):
        ...
        servdata = ServerData(message.source_id, message.server_name, message.server_info) 
        servdata.first_link_id = message.last_hop_id

        if message.last_hop_id == self.id or message.last_hop_id == 0:
            servdata.first_link_id = message.source_id
            self.sel.modify(io_device.fileobj, selectors.EVENT_READ | selectors.EVENT_WRITE, servdata)

        if message.last_hop_id == 0:
            reg_msg = ServerRegistrationMessage.bytes(self.id, message.source_id, self.server_name, self.server_info) 
            servdata.write_buffer += reg_msg
            for connected in self.hosts_db.values():
                if isinstance(connected, ServerData):
                    server_reg_msg = ServerRegistrationMessage.bytes(connected.id, self.id, connected.server_name, connected.server_info)
                    servdata.write_buffer += server_reg_msg
                elif isinstance(connected, ClientData):
                    client_reg_msg = ClientRegistrationMessage.bytes(connected.id, self.id, connected.client_name, connected.client_info)
                    servdata.write_buffer += client_reg_msg 

        self.hosts_db[message.source_id] = servdata
        server_reg_mess = ServerRegistrationMessage.bytes(message.source_id, self.id, self.server_name, self.server_info) 
        self.broadcast_message_to_servers(server_reg_mess, ignore_host_id=message.last_hop_id)
    ...
    def handle_client_registration_message(self, io_device, message):
        ...
        if message.last_hop_id == self.id or message.last_hop_id == 0:
            ...
            upmessage = StatusUpdateMessage.bytes(self.id, message.source_id, 0x00, "Welcome to the Clemson Relay Chat network " + str(client_data.client_name))
            client_data.write_buffer += upmessage
            

        if message.last_hop_id == 0:
            for connected in self.hosts_db.values():
                if isinstance(connected, ClientData):
                    reg_msg = ClientRegistrationMessage.bytes(connected.id, self.id, connected.client_name, connected.client_info)
                    client_data.write_buffer += reg_msg


        self.hosts_db[message.source_id] = client_data
        server_reg_mess = ClientRegistrationMessage.bytes(message.source_id, self.id, message.client_name, message.client_info)
        self.broadcast_message_to_servers(server_reg_mess, ignore_host_id = message.last_hop_id)  
        self.broadcast_message_to_adjacent_clients(server_reg_mess, ignore_host_id = message.last_hop_id)
    ...
    def handle_status_message(self, select_key, message):
        # TODO: Implement the above functionality
        if (message.destination_id is self.id or message.destination_id == 0) == True:
            self.status_updates_log.append(message.content)
        elif message.destination_id:
            self.send_message_to_host(message.destination_id, message)
    ...
    def handle_client_chat_message(self, select_key, message):
        ...
        if (message.destination_id in self.hosts_db):
            msg = ClientChatMessage.bytes(message.source_id, message.destination_id, message.content)
            self.send_message_to_host(message.destination_id, msg)
            
        else:
            status = StatusUpdateMessage.bytes(self.id, message.source_id, 0x01, "Unknown ID " + str(message.destination_id))
            self.send_message_to_host(message.source_id, status)
        self.status_updates_log.append(message.bytes) 
    ...
    def handle_client_quit_message(self, io_device, message):
        ...
        if message.source_id in self.hosts_db.keys():
            if(io_device.data is ServerData):
                self.broadcast_message_to_servers(message.bytes, ignore_host_id=None)
                self.broadcast_message_to_adjacent_clients(message.bytes)
            else:
                self.broadcast_message_to_servers(message.bytes)
                self.broadcast_message_to_adjacent_clients(message.bytes, ignore_host_id = io_device.data.id)
